...
namespace NAuth.Test.Domain
{
    public class LocalAuthHandlerTests
    {
        private readonly Mock<IOptionsMonitor<AuthenticationSchemeOptions>> _mockOptions;
        private readonly Mock<ILoggerFactory> _mockLoggerFactory;
        private readonly Mock<ILogger<LocalAuthHandler>> _mockLogger;
        ...
        public LocalAuthHandlerTests()
        {
            _mockOptions = new Mock<IOptionsMonitor<AuthenticationSchemeOptions>>();
            _mockLoggerFactory = new Mock<ILoggerFactory>();
            _mockLogger = new Mock<ILogger<LocalAuthHandler>>();
            ...
            _scheme = new AuthenticationScheme("TestScheme", "TestScheme", typeof(LocalAuthHandler));
        ...
        private LocalAuthHandler CreateHandler()
        {
            return new LocalAuthHandler(
                _mockOptions.Object,
        ...
        private async Task<LocalAuthHandler> InitializeHandlerAsync(HttpContext context)
        {
        ...
        public async Task HandleAuthenticateAsync_WithWhitespaceAuthorizationHeader_ShouldFail()
        {
            ...
            // Assert
            Assert.False(result.Succeeded);
            // Com apenas espaços, o Parse tenta processar e falha com erro de validação
        ...
        public async Task HandleAuthenticateAsync_WithValidJwtToken_AndUserExists_ShouldSucceed()
        {
            ...
            mockUser.SetupGet(u => u.UserId).Returns(userId);
        ...
        public async Task HandleAuthenticateAsync_WithNonHmacSha256Token_ShouldFail()
        {
            // Arrange
            var tokenHandler = new JwtSecurityTokenHandler();
            // Precisa de uma chave maior para HS512 (64 bytes ao invés de 32)
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                ...
                SigningCredentials = new SigningCredentials(
                    new SymmetricSecurityKey(key),
                    SecurityAlgorithms.HmacSha512 // Algoritmo diferente
                ),
                Issuer = "NAuth",
                Audience = "NAuth.API"
            };
            ...
            // Assert
            Assert.False(result.Succeeded);
            // O token com algoritmo diferente é rejeitado (pode ser por assinatura ou formato)
        ...
        [Fact]
        public async Task HandleAuthenticateAsync_WhenUserServiceThrows_ShouldFail()
        {
            // Arrange
            var userId = 1L;
            var token = GenerateValidJwtToken(userId, "Test User", "test@test.com");
        ...
        public async Task HandleAuthenticateAsync_WithJustExpiredToken_ShouldFailImmediately()
        {
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                ...
                Expires = now.AddSeconds(-1), // Expirou 1 segundo atrás
