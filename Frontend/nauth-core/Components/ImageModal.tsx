  ...
  const uploadImage = async () => {
    ...
    switch (param.Image) {
      case ImageTypeEnum.User:
        var ret = await userContext.uploadImageUser(blob);
        if (ret.sucesso) {
          if (param.onSuccess) {
    ...
    <Modal show={param.show} size="lg" onHide={param.onClose}>
      ...
      <Modal.Body>
        <div>
          ...
          {src && (
            <ReactCrop crop={crop} aspect={param.Image == ImageTypeEnum.User ? 1 : null} onChange={(newCrop) => setCrop(newCrop)}>
      ...
      <Modal.Footer>
        ...
        <Button
          ...
          {userContext.loading ? 'Saving...' : 'Upload Photo'}
        </Button>
      </Modal.Footer>
    </Modal>
  );
