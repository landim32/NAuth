  ...
  return (
    <ToastContainer className="p-3" position="bottom-end" style={{ zIndex: 9999 }}>
      <Toast animation={true} onClose={param.onClose} show={param.showMessage} delay={10000} autohide>
        <Toast.Header closeButton={true}>{showTitle(param.dialog)}</Toast.Header>
        <Toast.Body>
          {param.messageText}
          {param.dialog == MessageToastEnum.Confirmation && (
            <div className="mt-2 pt-2 border-top">
              <Button variant="success" size="sm" onClick={param.onYes}>
                Yes
              </Button>
              &nbsp;
              <Button variant="danger" size="sm" onClick={param.onNo}>
                No
              </Button>
            </div>
          )}
